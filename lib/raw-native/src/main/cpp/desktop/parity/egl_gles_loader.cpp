...
namespace
{
       ...
       void *razResolve(const char *name)
       {
              ...
              if (!p)
                     p = (void *)GetProcAddress(g_egl, name);
              if (!p)
              {
...
RAZ_GL(void, glTexImage2D, PFNGLTEXIMAGE2DPROC,
       ...
       (t, l, i, w, h, b, f, ty, d))
RAZ_GL(void, glTexImage3D, PFNGLTEXIMAGE3DPROC,
       (GLenum t, GLint l, GLint i, GLsizei w, GLsizei h, GLsizei d, GLint b,
        GLenum f, GLenum ty, const void *p),
       (t, l, i, w, h, d, b, f, ty, p))
