...
bool loadCubeLut(const std::string& cubePath, LutData& out, std::string& err) {
    ...
    if (c.size <= 0 || c.rgb.empty()) { err = "parseCubeFile failed or empty: " + cubePath; return false; }
    out.rgb  = std::move(c.rgb);
    out.size = c.size;
    for (int i = 0; i < 3; ++i) { out.domainMin[i] = c.domainMin[i]; out.domainMax[i] = c.domainMax[i]; }
    return true;
}
...
bool loadLutFile(const std::string& lutPath, LutData& out, std::string& err) {
    ...
    if (ext == "xmp" || ext == "lrtemplate") {
        ...
        if (!file.is_open()) { err = "Cannot open: " + lutPath; return false; }
        ...
        if (cubeString.empty()) { err = "Failed to convert Lightroom preset: " + lutPath; return false; }

        // Write cube string to a temporary file
        std::string tempPath = lutPath + ".tmp.cube";
        std::ofstream tempFile(tempPath, std::ios::binary);
        if (!tempFile.is_open()) { err = "Cannot write temp file: " + tempPath; return false; }
        tempFile << cubeString;
        tempFile.close();
...
static bool downsampleStageA(const std::string& srcTif, const std::string& dstTif,
    ...
    if (!r) { err = "openStageATiff failed: " + srcTif; return false; }
    const raw_v3::StageATiffHeader& h = raw_v3::getStageATiffHeader(r);
    const uint32_t sw = h.width, sh = h.height;
    if (sw == 0 || sh == 0) { raw_v3::closeStageATiff(r); err = "zero source dims"; return false; }
...
LensProbeResult probeLensProfile(const std::string& inputPath, const std::string& lensfunDbDir) {
    ...
    if (dot != std::string::npos) {
        ...
        for (auto& c : r.formatExt) c = (char)tolower((unsigned char)c);
    }
    ...
    if (!lensfunDbDir.empty()) {
        ...
        if (db) {
            ...
            if (m.lens) r.matchedLensModel = m.lens->model;
        }
    }
    return r;
}
...
std::vector<LensfunDbEntry> lensfunCameraList(const std::string& lensfunDbDir) {
    ...
    if (lensfunDbDir.empty()) return out;
    const LfDatabase* db = lfa_cached_database(lensfunDbDir.c_str());
    if (!db) return out;
    out.reserve(db->cameras.size());
    for (auto& c : db->cameras) out.push_back({c.maker, c.model, c.cropFactor});
    return out;
}
...
std::vector<LensfunDbEntry> lensfunLensList(const std::string& lensfunDbDir) {
    ...
    for (auto& l : db->lenses) out.push_back({l.maker, l.model, 0.f});
    return out;
}
...
void RazEngine::close() {
    if (!workTif_.empty())  std::remove(workTif_.c_str());
    if (!proxyTif_.empty()) std::remove(proxyTif_.c_str());
    workTif_.clear(); proxyTif_.clear();
...
float RazEngine::probeImage(const uint8_t* rgba, int width, int height) {
    if (!aiProbe_ || !aiProbe_->isReady()) { lastAiScore_ = -1.f; return -1.f; }
...
OpenResult RazEngine::open(const std::string& inputPath, int proxyLongSide,
    ...
    srcW_ = meta_.width; srcH_ = meta_.height;
...
raw_v3::StageCOptions RazEngine::makeOptions(const float* params, int count,
    ...
    if (lut && !lut->empty()) {
        ...
        o.lutSize = lut->size;
        for (int i = 0; i < 3; ++i) { o.lutDomainMin[i] = lut->domainMin[i]; o.lutDomainMax[i] = lut->domainMax[i]; }
    }
...
bool RazEngine::renderProxy(const float* params, int count, const LutData* lut,
    ...
    if (proxyTif_.empty()) { err = "no image open"; return false; }
    ...
    if (!sc.success) { err = sc.error.empty() ? "runStageCToRGBA8 failed" : sc.error; return false; }
...
bool RazEngine::exportTiff16(const float* params, int count, const LutData* lut,
    ...
    if (workTif_.empty()) { err = "no image open"; return false; }
    raw_v3::StageCExif exif{};
    raw_v3::StageCOptions o = makeOptions(params, count, lut, exif);
    o.format = raw_v3::StageCFormat::Tiff16;
    raw_v3::StageCResult sc = raw_v3::runStageC(workTif_, outPath, o);
    if (!sc.success) { err = sc.error.empty() ? "runStageC failed" : sc.error; return false; }
    return true;
}
...
bool RazEngine::exportJpegOrPng(const float* params, int count, const LutData* lut,
    ...
    raz::EncodeMeta m{};
    m.make = meta_.cameraMake; m.model = meta_.cameraModel; m.lensModel = meta_.lensModel;
    m.dateTimeOriginal = meta_.dateTimeOriginal; m.dateTime = meta_.dateTimeOriginal;
    m.iso = meta_.iso; m.exposureTime = meta_.shutterSpeed;
    m.fNumber = meta_.aperture; m.focalLength = meta_.focalLength;
