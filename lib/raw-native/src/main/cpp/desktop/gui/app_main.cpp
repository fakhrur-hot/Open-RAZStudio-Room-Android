                               ...
                               const std::vector<std::pair<int, float>> &adjustments)
{
    ...
    {
        if (slot >= 0 && slot < (int)params.size())
            params[slot] += value;
    }
}
...
static std::vector<LutCategory> scanLutLibrary()
{
    ...
    if (GetFileAttributesA(lutRoot.c_str()) == INVALID_FILE_ATTRIBUTES)
    {
        lutRoot = exeDir() + "\\..\\..\\..\\engine\\luts";
    }
    ...
    if (hR != INVALID_HANDLE_VALUE)
    {
        ...
        FindClose(hR);
        if (!misc.entries.empty())
            cats.insert(cats.begin(), misc);
    }
    return cats;
}
...
static void doOpen(App &a, const std::string &path)
{
    ...
    case 0:
        sa.highlightMode = 0;
        sa.highlightDesaturateStrength = 0.f;
        break; // Off
    case 1:
        sa.highlightMode = 2;
        sa.highlightDesaturateStrength = 0.f;
    ...
    default:
        sa.highlightMode = 2;
        sa.highlightDesaturateStrength = 1.f;
        break; // Safe
    }
...
static bool drawAdjustmentPanel(App &a)
{
    ...
    if (ImGui::BeginPopupModal("Lens & Format Check", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        else
        {
            ...
            if (a.lensProbe.focalMm > 0.f)
                ImGui::Text("Focal length: %.0f mm", a.lensProbe.focalMm);
            else
                ImGui::TextDisabled("Focal length: not reported (manual/adapted lens)");
            if (a.lensProbe.aperture > 0.f)
                ImGui::Text("Aperture: f/%.1f", a.lensProbe.aperture);
        }
        ImGui::Separator();
    ...
    ImGui::SameLine();
    ImGui::Checkbox("Quick Edit", &a.quickEdit);
    ImGui::Separator();
    ...
    if (a.quickEdit)
    {
        ImGui::TextDisabled("Quick Edit");
        ImGui::TextWrapped("Core controls from the Android light and color workflow.");
        dirty |= SlotSlider("Exposure (EV)##quickexp", a.params.data(), 0, -4.f, 4.f);
        dirty |= SlotSlider("Contrast##quickcontrast", a.params.data(), 1, -1.f, 1.f);
        dirty |= SlotSlider("Temperature (K delta)##quickwb", a.params.data(), 8,
                            -3500.f, 6500.f, "%.0f");
        dirty |= SlotSlider("Tint##quicktint", a.params.data(), 9, -200.f, 200.f, "%.0f");
        ImGui::Separator();
        if (ImGui::Button("Open Full Adjustments"))
            a.quickEdit = false;
        ImGui::EndDisabled();
        ImGui::Separator();
    ...
    if (ImGui::BeginTabBar("##adj"))
    {
        ...
        if (ImGui::BeginTabItem("Tone"))
        {
            ...
            if (ImGui::Button("-##aiexpminus"))
            {
                ...
                {
                    if (slot >= 0 && slot < (int)a.params.size())
                        a.params[slot] -= value;
                }
                a.aiExposeApplied.clear();
        ...
        if (ImGui::BeginTabItem("FX"))
        {
            ...
            ImGui::TextDisabled("Diffuser / Mist");
