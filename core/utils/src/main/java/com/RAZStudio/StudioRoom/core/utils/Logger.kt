...
import com.RAZStudio.StudioRoom.core.utils.Logger.Level
import com.RAZStudio.StudioRoom.core.utils.Logger.reportError
import com.RAZStudio.StudioRoom.core.utils.LogsWriter.Companion.MAX_SIZE
import com.RAZStudio.StudioRoom.core.utils.LogsWriter.Companion.STARTUP_LOG
...
inline fun <reified T> makeLog(
    ...
    if (!AppLogger.isEnabled) return
    Logger.makeLog(tag = tag, level = level, dataBlock = { data })
}
...
fun makeLog(
    ...
    if (!AppLogger.isEnabled) return
    Logger.makeLog(level = level, dataBlock = { data.toList().joinToString(separator) })
}
...
inline fun <reified T> T.makeLog(
    ...
    if (!AppLogger.isEnabled) return@also
    if (it is Throwable) {
